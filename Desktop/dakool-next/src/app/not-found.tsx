  ...
  return (
    <section className="grain relative flex min-h-[100svh] items-center overflow-hidden bg-bg">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        ...
        <span className="absolute top-1/2 right-4 hidden -translate-y-1/2 font-display text-[26rem] leading-none text-fg/[0.03] select-none lg:block">
          404
        </span>
      </div>

      <Container className="relative z-10 py-32">
        <span className="mb-6 flex items-center gap-3 text-[11px] font-bold uppercase tracking-brand text-fg">
        ...
        <div className="grid max-w-3xl grid-cols-1 gap-px border border-line bg-line sm:grid-cols-2">
          {suggestions.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="group bg-bg p-5 transition-colors hover:bg-elevated"
            >
              <span className="block font-display text-xl text-fg">{item.label}</span>
